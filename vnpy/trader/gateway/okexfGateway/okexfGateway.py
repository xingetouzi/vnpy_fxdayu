...
class OkexfGateway(VtGateway):
    ...
    def connect(self):
        ...
        if "_BTC" in self.contracts or "_USDT" in self.contracts:
    ...
    def subscribe(self, subscribeReq):
        ...
        if "this_week" in symbol or "quarter" in symbol or "next_week" in symbol:
...
class OkexSwapWebsocketApi(WebsocketClient):
    ...
    def subscribe(self, symbol):
        """"""
        # 订阅TICKER
        # channel1 = 'swap/ticker:%s' %(actType)
        # self.callbackDict[channel1] = self.onSwapTick
        # self.channelSymbolDict[channel1] = symbol
        ...
        self.callbackDict[channel2] = self.onSwapDepth
        self.channelSymbolDict[channel2] = symbol
        ...
        self.callbackDict[channel3] = self.onSwapTrades
        self.channelSymbolDict[channel3] = symbol
        ...
        self.callbackDict[channel4] = self.onSwapPriceRange
    ...
    def onLogin(self, d):
        ...
        for contract in self.gateway.contracts:
            self.subscribe(contract)
            self.sendPacket({'event': 'subscribe', 'channel': 'swap/position:%s'%contract})
            self.sendPacket({'event': 'subscribe', 'channel': 'swap/account:%s'%contract})
            self.sendPacket({'event': 'subscribe', 'channel': 'swap/order:%s'%contract})
    #----------------------------------------------------------------------
    def onSwapTick(self, d):
        ...
        for n, data in enumerate(d['data']):
            tick = copy(self.tickDict[data['instrument_id']])
            ...
            # tick.lowerLimit = float(data['limitLow'])
            # tick.openInterest = float(data['hold_amount'])
            tick.datetime = datetime.strptime(data['timestamp'],"%Y-%m-%dT%X.%fZ")
            tick.date = tick.datetime.strftime('%Y%m%d')
            tick.time = tick.datetime.strftime('%H:%M:%S.%f')
            tick.localTime = datetime.now()
            tick.volumeChange = 0
    ...
    def onTrade(self, d):
        ...
        for n, data in enumerate(d['data']):
            ...
            order.orderDatetime = datetime.strptime(data['timestamp'],"%Y-%m-%dT%X.%fZ")
