...
class CtpGateway(VtGateway):
    ...
    #----------------------------------------------------------------------
    def __init__(self, eventEngine, gatewayName='HQ'):
        """Constructor"""
    ...
    def loadHistoryBar(self, vtSymbol, type_, size=None, since=None):
        ...
        if self.dbURI and not self.jaqsUser:
            ...
            if type_ not in ['1min','5min','15min']:
                self.mdApi.writeLog(u'CTP初始化数据只接受1分钟,5分钟，15分钟bar')
                return
...
class CtpTdApi(TdApi):
    ...
    def sendOrder(self, orderReq):
        ...
        order = self.gateway.makeOrder(oid, orderReq.__dict__)
        self.gateway.pendingOrder.update({oid : order})

        # 返回订单号（字符串），便于某些算法进行动态管理
        self.writeLog('Gateway 发单：%s'%oid)
